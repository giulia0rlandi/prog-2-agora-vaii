...
print(g1b.agg({'TAXA_CRESCIMENTO':'mean'}))
...
df1c = g1b.agg({'TAXA_CRESCIMENTO':'mean'})
df1c.plot.bar()
plt.show()
...
mport = dfDesempenhoEscolar['NOTA_MATEMATICA'].mean(axis=0)
mmat = dfDesempenhoEscolar['NOTA_PORTUGUES'].mean(axis=0)
...
f1e= dfDesempenhoEscolar[['NOTA_MATEMATICA','NOTA_PORTUGUES']].idxmin(axis=1)
print(f1e)

print('\n------------------------------------------------------')
print('1.f- Gráfico de pizza da disciplina de pior desempenho, por cidade ( Matemática ou Português')
print('------------------------------------------------------')

qtd = f1e.value_counts()
qtd.plot.pie()
plt.show()
...
ricos =dfIndicadoresSociais[dfIndicadoresSociais['POBREZA']==dfIndicadoresSociais['POBREZA'].min()]
print(ricos.index[0])
...
coluninha = pd.cut(dfCidades['IDH'],bins = 4, labels = ['Baixo', 'Médio', 'Alto', 'Muito Alto'])
dfCidades['CatIDH']=coluninha
...
col2b = dfDesempenhoEscolar[['NOTA_MATEMATICA','NOTA_PORTUGUES']].mean(axis=1)
dfDesempenhoEscolar['Desempenho_Médio']=col2b
...
f3a = dfCidades[dfCidades['CatIDH']=='Muito Alto'].index

print(dfDesempenhoEscolar.loc[f3a]['Desempenho_Médio'].mean())
...
f4b = dfCompleto[dfCompleto['REGIAO'] == 'Norte']
menor = f4b[f4b['TAXA_CRESCIMENTO']==f4b['TAXA_CRESCIMENTO'].min()]
print(menor.index[0])
...
# g4c = dfCompleto.groupby('CatIDH')['Nível_Pobreza']
...
f4d1 = dfCompleto[dfCompleto.DESIGUALDADE>0.5]

print(f4d1[['NOTA_MATEMATICA','NOTA_PORTUGUES']].mean())
...
tabfreq = pd.crosstab(f5c['REGIAO'],f5c['Nível_Pobreza'], values =f5c['TAXA_APROVACAO'], aggfunc='mean' )
print(tabfreq)
...
f5d = dfCompleto[(dfCompleto['TAXA_APROVACAO']==dfCompleto['TAXA_APROVACAO'].min())&(dfCompleto['RENDA_MEDIA']>2500)]
...
f5g = dfCompleto[(dfCompleto['RENDA_MEDIA']>3000)&(dfCompleto['TAXA_APROVACAO']<80)]
print(len(list(f5g.index)))
